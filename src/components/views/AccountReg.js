...
function AccountReg(props) {
  ...
  const verifyEmail = () => {
    console.log("Verifying email:", email, "with locale:", (localStorage.getItem('locale') || "Mn"));
    return axios.post(serverUrl + "users/emailverify", {
      ...
      if (response.data.response) {
        ...
        setEmailMessage({ style: 'text-green-500', val: 1, data: t("E-mail successfully verified!") })
      } else {
        openNotification(t('Fail!'), t("E-mail not verified!"), false, false)
        setEmailMessage({ style: 'text-red-500', val: 0, data: t("E-mail not verified!") })
      }
    });
  }
      ...
      .then((values) => {
        axios.post(serverUrl + "users/signup", {
          ...
          if (response.data.response) {
            ...
            setEmailMessage({ style: 'text-green-500', val: true, data: t("Account successfully created!") })
            ...
            setEmailMessage({ style: 'text-red-500', val: false, data: t("Account failed!") })
          }
        })
      })
  }
      ...
      >
        ...
        >
          <Input
            ...
            suffix={<Button onClick={() => hasErrors(form.getFieldsError())} className="bg-gray-800 text-md text-white rounded-xl py-1 px-4">{t("Send")}</Button>}
