...
class EmailVerifyModel {
    ...
    create = async ({ email, verify_code }) => {
        try {
            console.log('Attempting to create record with Email:', email, 'and Verification Code:', verify_code);
            
            if (!email ||!verify_code) {
                throw new Error('Email or verify_code is undefined');
            }
            
            const sql = `INSERT INTO ${this.tableName} (email, verify_code) VALUES (?,?)`;
            
            const result = await query(sql, [email, verify_code]);
            const affectedRows = result? result.affectedRows : 0;
            
            console.log('Record created successfully. Affected rows:', affectedRows);
            return affectedRows;
        } catch (error) {
